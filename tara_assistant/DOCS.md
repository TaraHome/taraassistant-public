...
Tara includes an AI-powered device onboarding agent that helps set up new devices
added to Home Assistant.

### How it works

- A background scanner periodically checks for unconfigured or newly added devices
- When new devices are found, Tara suggests friendly names, areas, and automations
- You can review and approve all suggestions before they are applied

### Configuration

| Option | Default | Description |
|--------|---------|-------------|
| `onboarding_scan_enabled` | `true` | Enable or disable the background device scanner |
| `onboarding_scan_interval` | `300` | Scan interval in seconds (5 minutes default) |

### Web Search

Tara can search the web to find device documentation, manuals, and setup guides when
onboarding unfamiliar devices. By default, DuckDuckGo is used. To enable Brave Search,
set your `brave_search_api_key` in the add-on configuration.
